...
def get_ngrams(tokens):
    ...
    Returns a dictionary of n-grams with their frequencies, sorted by frequency and truncated to the top 15%.
    """
    ...
    sorted_ngrams_truncated_length = int(0.15 * len(sorted_ngrams))
...
def main():
    ...
    # Create ngrams.json : n-grams (bigrams and trigrams) with their frequencies from filtered words sequence, truncated to the top 15%.
    output_file = os.path.join(path, "ngrams.json")
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(ngrams, f, ensure_ascii=False, indent=4)
    print(f"File created: {output_file}")

    # Create ngrams_strong.json : n-grams from words sequence as is, truncated to the top 15%.
