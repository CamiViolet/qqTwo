...
def get_activity_size(directory):
...
def get_activity_date(activity_path):
    """Get the most recent update date for an activity.

    Returns the date as a string in 'YYYY-MM-DD' format (e.g. '2024-03-15'),
    or None if no date properties are found in any markdown file.
    """
    md_files = glob.glob(os.path.join(activity_path, "*.md"))
    dates = []
    ...
    for i, line in enumerate(lines):
        ...
        if prop_name in prop_names_for_dates:
            ...
            for p in props:
                ...
                if re.match(r'^\d{4}-\d{2}-\d{2}( \d{2}:\d{2})?$', p):
                    new_props.append(p)
            props = new_props
